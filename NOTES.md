...
# Corrections and changes, 2026-08-20 (wiring session)

Everything above is the delivering session's own account. Four of its claims
did not survive checking against the app, and three things changed in the
data. Recorded here rather than edited above, so the original reasoning stays
readable.

## 1. The app's text is not the WEB (§2 is wrong)

`bible.json` is not the World English Bible. Genesis 1:1, Psalms 23:1 and
Matthew 5:3 are verbatim KJV. `app.js` has said so in Lulo's own prompt the
whole time: the file is *not one translation* — Old Testament in older
English, New Testament in modern. The one verse that led §2 astray
(John 3:16, "his only born Son") is from the modern half.

## 2. Coverage is much better than "40.4%" suggests

That figure divides tags by *all* English words, but the KJVA source only
contains 355,850 tag elements in the 66 books at all — one `<w>` tag spans a
whole phrase ("In the beginning" is a single `H7225`), so most words could
never have carried one. Measured against what was actually available:

| | available | kept | retained |
|---|---|---|---|
| OT | 227,196 | 224,793 | **98.9%** |
| NT | 128,654 | 92,704 | **72.1%** |
| all | 355,850 | 317,497 | 89.2% |

The Old Testament is as complete as this source permits. The real gap is the
New, and it is concentrated: 3 John 57%, Philemon 60%, Jude 61%,
James / 1 Peter / Luke 64%, Revelation 65%. The pipeline behaved correctly
throughout; only the explanation was wrong. The reasoning given for the three
zero-tag verses is wrong for the same reason.

## 3. `gloss` was the wrong field (fixed — `scripts/fix_glosses.py`)

`build_lexicon.py` took the dictionary's *first-listed* KJV rendering. Strong's
lists renderings alphabetically, so that is frequently a rare sense:

    G2889 kosmos  -> "adorning"    (renders "world" 181x)
    G3056 logos   -> "account"     (renders "word" 166x)
    G5485 charis  -> "acceptable"  (renders "grace" 129x)
    H2617 chesed  -> "favour"      (renders "mercy" 137x)

Those are the words people ask about most. Now set to the rendering each
number actually carries most often in this text, gated on the dictionary's own
rendering list so nothing is invented. 8,711 entries changed; 1,425 with no
tagged occurrence were left untouched.

## 4. `translit` was a pronunciation key, renamed to `pron`

The field held `ag-ap-ah'-o`, not `agapao`. Renamed for what it is rather than
shipping a label its contents contradict. It is the better field for voice
anyway. A real transliteration is still missing and would have to be derived
from the lemma.

## 5. Tags now carry the word's index (`scripts/build_tagged_bible.py`)

Each tag is `{i, w, strongs}`. `i` was already computed inside `align_verse`
and simply not written out. Without it a consumer has to re-match on the
word's text, and a verse saying "God" twice under two different numbers gets
one of them attached to the wrong occurrence — 2,077 verses are exposed to
that. Verified: all 317,497 indices resolve to the right word, strictly
increasing, no collisions.

## 6. Shipping layout (`scripts/split_for_phone.py`)

14MB in one payload was never the network problem it looks like (2.2MB
gzipped) — it is the parse. 317,497 tiny objects leave tens of MB resident on
a phone already holding `bible.json`, the wave canvas and decoded audio. Split
instead:

    data/tagged/<book>.json   66 files, median 7KB gz, largest Psalms 55KB gz
    data/lexicon-index.json   258KB gz   eager
    data/lexicon-defs.json    544KB gz   lazy, first word tapped

Eager cost: 2.2MB gz -> 258KB gz. Opening a chapter costs ~7KB. The shipped
tag files drop `w` (44% of the payload, and recoverable as `verse.split()[i]`);
the pipeline's own `output/` copy keeps it, where it is what makes the tagging
auditable by eye.

## 7. Still open

- The GPL question in §1 is unresolved. Accepted for now as a deliberate
  decision, not an oversight.
- 34 of the 61 hand-written `lulo-lexicon.js` entries have no Strong's number
  attached, so their cautions never fire. Linking by sound alone is unsafe —
  it proposed `eros`->"part", `abba`->"servants", `torah`->"cumbrance". The 27
  that are linked were each checked by hand against that entry's own
  definition.
